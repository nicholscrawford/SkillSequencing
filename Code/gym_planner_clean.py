...
class BehaviorRunner:
    ...
    def __init__(self, model, session_config, rate, use_reflex, taskcfgdict, fake=False):
    ...
    def run_behavior(self, preview=True):
        ...
        action_idx, action_param = sample_for_action_param(self.model, pc_tensor, desired_pc) # Desired pc?

        #self.behavior.set_target... Apply action param according to behavior/action, maybe reinitialize behavior
        
        # selected_behavior = self.taskcfgdict[list(self.taskcfgdict.keys())[action_idx]]
        selected_behavior = self.taskcfgdict["PullFromShelf"]
...
if __name__ == "__main__":
    ...
    runner = BehaviorRunner(model, session_config, args.rate, args.use_reflex, taskcfgdict, args.fake)
